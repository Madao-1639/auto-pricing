...
class LjspiderSpider(scrapy.Spider):
    ...
    def infoparse(self, response):
        ...
        areaName = response.xpath('//div[class="aroundInfo"]/div[@class="areaName"]/span[@class="class="info"]/a/text()').getall()
        region = areaName[0]
        if len(areaName)>1:
            garden = areaName[1]
        ...
        usage = transaction_info.xpath('./span[text()="房屋用途"]/following-sibling::text()').get()
        year = transaction_info.xpath('./span[text()="房屋年限"]/following-sibling::text()').get()
        #ownership = transaction_info.xpath('./span[text()="产权所属"]/following-sibling::text()').get()
        #mortgage = transaction_info.xpath('./span[text()="抵押信息"]/following-sibling::text()').get()
